...
*       layouts must be defined manually on controls drawing, providing the right bounds Rect for it.
*
...
RAYGUIAPI int GuiWindowBox(Rect bounds, const char *title);                                       // Window Box control, shows a window that can be closed
RAYGUIAPI int GuiGroupBox(Rect bounds, const char *text);                                         // Group Box control with text name
RAYGUIAPI int GuiLine(Rect bounds, const char *text);                                             // Line separator control, could contain text
RAYGUIAPI int GuiPanel(Rect bounds, const char *text);                                            // Panel control, useful to group controls
RAYGUIAPI int GuiTabBar(Rect bounds, const char **text, int count, int *active);                  // Tab Bar control, returns TAB to be closed or -1
RAYGUIAPI int GuiScrollPanel(Rect bounds, const char *text, Rect content, Vector2 *scroll, Rect *view); // Scroll Panel control

// Basic controls set
RAYGUIAPI int GuiLabel(Rect bounds, const char *text);                                            // Label control, shows text
RAYGUIAPI int GuiButton(Rect bounds, const char *text);                                           // Button control, returns true when clicked
RAYGUIAPI int GuiLabelButton(Rect bounds, const char *text);                                      // Label button control, show true when clicked
RAYGUIAPI int GuiToggle(Rect bounds, const char *text, bool *active);                             // Toggle Button control, returns true when active
RAYGUIAPI int GuiToggleGroup(Rect bounds, const char *text, int *active);                         // Toggle Group control, returns active toggle index
RAYGUIAPI int GuiCheckBox(Rect bounds, const char *text, bool *checked);                          // Check Box control, returns true when active
RAYGUIAPI int GuiComboBox(Rect bounds, const char *text, int *active);                            // Combo Box control, returns selected item index

RAYGUIAPI int GuiDropdownBox(Rect bounds, const char *text, int *active, bool editMode);          // Dropdown Box control, returns selected item
RAYGUIAPI int GuiSpinner(Rect bounds, const char *text, int *value, int minValue, int maxValue, bool editMode); // Spinner control, returns selected value
RAYGUIAPI int GuiValueBox(Rect bounds, const char *text, int *value, int minValue, int maxValue, bool editMode); // Value Box control, updates input text with numbers
RAYGUIAPI int GuiTextBox(Rect bounds, char *text, int textSize, bool editMode);                   // Text Box control, updates input text

RAYGUIAPI int GuiSlider(Rect bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue); // Slider control, returns selected value
RAYGUIAPI int GuiSliderBar(Rect bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue); // Slider Bar control, returns selected value
RAYGUIAPI int GuiProgressBar(Rect bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue); // Progress Bar control, shows current progress value
RAYGUIAPI int GuiStatusBar(Rect bounds, const char *text);                                        // Status Bar control, shows info text
RAYGUIAPI int GuiDummyRec(Rect bounds, const char *text);                                         // Dummy control for placeholders
RAYGUIAPI int GuiGrid(Rect bounds, const char *text, float spacing, int subdivs, Vector2 *mouseCell); // Grid control, returns mouse cell position
...
RAYGUIAPI int GuiListViewEx(Rect bounds, const char **text, int count, int *scrollIndex, int *active, int *focus); // List View with extended parameters
...
RAYGUIAPI int GuiTextInputBox(Rect bounds, const char *title, const char *message, const char *buttons, char *text, int textMaxSize, bool *secretViewActive); // Text Input Box control, ask for text, supports secret
RAYGUIAPI int GuiColorPicker(Rect bounds, const char *text, Color *color);                        // Color Picker control (multiple color controls)
RAYGUIAPI int GuiColorPanel(Rect bounds, const char *text, Color *color);                         // Color Panel control
RAYGUIAPI int GuiColorBarAlpha(Rect bounds, const char *text, float *alpha);                      // Color Bar Alpha control
RAYGUIAPI int GuiColorBarHue(Rect bounds, const char *text, float *value);                        // Color Bar Hue control
//----------------------------------------------------------------------------------------------------------
...
static Rect guiSliderActive = { 0 };       // Gui slider active bounds rectangle, used as an unique identifier
...
static void GuiTooltip(Rect controlRec);                   // Draw tooltip using control rec position
...
// Window Box control
int GuiWindowBox(Rect bounds, const char *title)
{
...
int GuiGroupBox(Rect bounds, const char *text)
{
...
// Line control
int GuiLine(Rect bounds, const char *text)
{
...
// Panel control
int GuiPanel(Rect bounds, const char *text)
{
...
int GuiTabBar(Rect bounds, const char **text, int count, int *active)
{
    ...
    Rect tabBounds = { bounds.x, bounds.y, RAYGUI_TABBAR_ITEM_WIDTH, bounds.height };
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (tabBounds.x < GetScreenWidth())
        {
            ...
            GuiSetStyle(BUTTON, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
#if defined(RAYGUI_NO_ICONS)
            if (GuiButton(RAYGUI_CLITERAL(Rect){ tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14 }, "x")) result = i;
#else
            if (GuiButton(RAYGUI_CLITERAL(Rect){ tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14 }, GuiIconText(ICON_CROSS_SMALL, NULL))) result = i;
#endif
            GuiSetStyle(BUTTON, BORDER_WIDTH, tempBorderWidth);
            GuiSetStyle(BUTTON, TEXT_ALIGNMENT, tempTextAlignment);
        }
    ...
    GuiDrawRectangle(RAYGUI_CLITERAL(Rect){ bounds.x, bounds.y + bounds.height - 1, bounds.width, 1 }, 0, BLANK, GetColor(GuiGetStyle(TOGGLE, BORDER_COLOR_NORMAL)));
    //--------------------------------------------------------------------
...
int GuiScrollPanel(Rect bounds, const char *text, Rect content, Vector2 *scroll, Rect *view)
{
    int result = 0;
    GuiState state = guiState;
...
// Label control
int GuiLabel(Rect bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiButton(Rect bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiLabelButton(Rect bounds, const char *text)
{
    GuiState state = guiState;
    bool pressed = false;
...
int GuiToggle(Rect bounds, const char *text, bool *active)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiCheckBox(Rect bounds, const char *text, bool *checked)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiDropdownBox(Rect bounds, const char *text, int *active, bool editMode)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiTextBox(Rect bounds, char *text, int bufferSize, bool editMode)
{
...
bool GuiTextBoxMulti(Rect bounds, char *text, int bufferSize, bool editMode)
{
    bool pressed = false;
...
int GuiSpinner(Rect bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    int result = 1;
    GuiState state = guiState;
...
int GuiValueBox(Rect bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    if (editMode)
    {
        ...
        Rect cursor = { bounds.x + GetTextWidth(textValue)/2 + bounds.width/2 + 1, bounds.y + 2*GuiGetStyle(VALUEBOX, BORDER_WIDTH), 4, bounds.height - 4*GuiGetStyle(VALUEBOX, BORDER_WIDTH) };
...
int GuiSliderPro(Rect bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue, int sliderWidth)
{
    int result = 0;
    GuiState state = guiState;
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (guiSliderDragging) // Keep dragging outside of bounds
        {
            ...
            else
            {
                ...
                guiSliderActive = RAYGUI_CLITERAL(Rect){ 0, 0, 0, 0 };
            }
        }
...
int GuiSlider(Rect bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue)
{
    return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue, GuiGetStyle(SLIDER, SLIDER_WIDTH));
}

// Slider Bar control extended, returns selected value
int GuiSliderBar(Rect bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue)
{
    return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue, 0);
}

// Progress Bar control extended, shows current progress value
int GuiProgressBar(Rect bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue)
{
    int result = 0;
    GuiState state = guiState;
...
// Status Bar control
int GuiStatusBar(Rect bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiDummyRec(Rect bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiListViewEx(Rect bounds, const char **text, int count, int *scrollIndex, int *active, int *focus)
{
    int result = 0;
    GuiState state = guiState;
...
// Color Panel control
int GuiColorPanel(Rect bounds, const char *text, Color *color)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiColorBarAlpha(Rect bounds, const char *text, float *alpha)
{
...
int GuiColorBarHue(Rect bounds, const char *text, float *hue)
{
    int result = 0;
    GuiState state = guiState;
...
int GuiColorPicker(Rect bounds, const char *text, Color *color)
{
    int result = 0;
...
int GuiTextInputBox(Rect bounds, const char *title, const char *message, const char *buttons, char *text, int textMaxSize, bool *secretViewActive)
{
    ...
    if (secretViewActive != NULL)
    {
        ...
        GuiToggle(RAYGUI_CLITERAL(Rect){ textBoxBounds.x + textBoxBounds.width - RAYGUI_TEXTINPUTBOX_HEIGHT, textBoxBounds.y, RAYGUI_TEXTINPUTBOX_HEIGHT, RAYGUI_TEXTINPUTBOX_HEIGHT }, (*secretViewActive == 1)? "#44#" : "#45#", secretViewActive);
    }
    else
    {
...
int GuiGrid(Rect bounds, const char *text, float spacing, int subdivs, Vector2 *mouseCell)
{
...
void GuiLoadStyleDefault(void)
{
    ...
    if (guiFont.texture.id != GetFontDefault().texture.id)
    {
        ...
        Rect whiteChar = guiFont.recs[95];
        // NOTE: We set up a 1px padding on char rectangle to avoid pixel bleeding on MSAA filtering
        SetShapesTexture(guiFont.texture, RAYGUI_CLITERAL(Rect){ whiteChar.x + 1, whiteChar.y + 1, whiteChar.width - 2, whiteChar.height - 2 });
    }
}
...
static void GuiLoadStyleFromMemory(const unsigned char *fileData, int dataSize)
{
        ...
        (signature[3] == ' '))
    {
        ...
        if (fontDataSize > 0)
        {
            ...
            memcpy(&whiteRec, fileDataPtr, sizeof(Rect));
            fileDataPtr += 16;
            ...
            {
                memcpy(&font.recs[i], fileDataPtr, sizeof(Rect));
                fileDataPtr += sizeof(Rect);
            }
...
static void GuiDrawText(const char *text, Rect bounds, int alignment, Color tint)
{
    ...
    if ((text != NULL) && (text[0] != '\0'))
    {
        ...
        //Rect textBounds = GetTextBounds(LABEL, bounds);
...
static void GuiTooltip(Rect controlRec)
{
    if (!guiLocked && guiTooltip && (guiTooltipPtr != NULL) && !guiSliderDragging)
    {
        ...
        GuiPanel(RAYGUI_CLITERAL(Rect){ controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, GuiGetStyle(DEFAULT, TEXT_SIZE) + 8.f }, NULL);

        int textPadding = GuiGetStyle(LABEL, TEXT_PADDING);
        int textAlignment = GuiGetStyle(LABEL, TEXT_ALIGNMENT);
        GuiSetStyle(LABEL, TEXT_PADDING, 0);
        GuiSetStyle(LABEL, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
        GuiLabel(RAYGUI_CLITERAL(Rect){ controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, GuiGetStyle(DEFAULT, TEXT_SIZE) + 8.f }, guiTooltipPtr);
...
static int GuiScrollBar(Rect bounds, int value, int minValue, int maxValue)
{
    ...
    arrowUpLeft = RAYGUI_CLITERAL(Rect){
    ...
    if (GuiGetStyle(SCROLLBAR, ARROWS_VISIBLE))
    {
        ...
        GuiDrawText(isVertical? "^" : "<",
            RAYGUI_CLITERAL(Rect){ arrowUpLeft.x, arrowUpLeft.y, isVertical? bounds.width : bounds.height, isVertical? bounds.width : bounds.height },
            TEXT_ALIGN_CENTER, Fade(GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state*3))), guiAlpha));
        GuiDrawText(isVertical? "v" : ">",
            RAYGUI_CLITERAL(Rect){ arrowDownRight.x, arrowDownRight.y, isVertical? bounds.width : bounds.height, isVertical? bounds.width : bounds.height },
            TEXT_ALIGN_CENTER, Fade(GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state*3))), guiAlpha));
#else
